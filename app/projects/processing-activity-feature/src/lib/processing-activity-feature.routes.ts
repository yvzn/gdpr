...
export const processingActivityFeatureRoutes: Routes = [
	{
		...
		providers: [
			provideState(processingActivityFeature),
			provideEffects(ProcessingActivityEffects),
		],
		children: [
			{
				path: '',
				loadComponent: () =>
					import('./views/activity-list/activity-list').then(
			...
			},
			{
				path: 'new',
				loadComponent: () =>
					import('./views/activity-create/activity-create').then(
